...
pub trait Resolver: ProofTree {
    ...
    fn resolve(&mut self, state: &mut LazyVCTState) -> Option<Mate> {
        self.resolve_attacks(state)
    }

    fn resolve_attacks(&mut self, state: &mut LazyVCTState) -> Option<Mate> {
    ...
    fn resolve_defences(&mut self, state: &mut LazyVCTState) -> Option<Mate> {
        if let Some(event) = state.check_event() {
            return match event {
