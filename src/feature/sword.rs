...
use crate::board::SequenceKind::Sword;
use crate::board::*;
...
/// (see [`SequenceKind::Sword`]); its two empty eyes are what a VCF search
...
/// `Board::sequences(r, Sword)` / `sequences_on(p, r, Sword)` would, in
...
impl SwordMap {
    ...
    fn update(&mut self, k: usize, board: &Board) {
        ...
        for r in [Black, White] {
            ...
            swords.starts[k] = line.sequence_starts(r, Sword);
...
fn swords_in(
    ...
    let (d, i) = Grid::line_of_key(k);
    let line = board.line(d, i).unwrap();
    bits(starts as u128).map(move |j| Sequence::new(Index::new(d, i, j), r, Sword, line.segment(j)))
}
...
mod tests {
    ...
    fn assert_matches_a_scan(map: &mut SwordMap, board: &Board) {
        map.sync(board);
        for r in [Black, White] {
            let scanned: Vec<_> = board.sequences(r, Sword).collect();
            assert_eq!(map.swords(board, r).collect::<Vec<_>>(), scanned, "{r:?}");
            for x in 0..RANGE {
                for y in 0..RANGE {
                    let p = Point(x, y);
                    let scanned: Vec<_> = board.sequences_on(p, r, Sword).collect();
                    assert_eq!(
