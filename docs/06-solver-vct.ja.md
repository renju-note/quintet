...
**方向ごとの値**（`Board::potentials(player, min, exact)`、02 §7）:

1. その点を通る 5 マスの窓のうち、相手の石を含まないものを取る。黒ではさらに `exact`（余白に自分の石がない = 長連にならない）を要求する。
2. 各窓について、そこに打った後に窓が含む自分の石の数を数える。`min` 以上なら残す。
3. `max ×（その max に達した窓の数）` を報告する。
...
- `min` は 2 つある。構築時の `2` は窓を選ぶ閾値、問い合わせ時の `3` は合計の閾値。
