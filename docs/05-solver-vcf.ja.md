...
3 つとも盤面を走査しない。`Board` がキャッシュしている剣先（`Board::swords` / `swords_on`、02 §8）を `Game::synced_board` 経由で読む。`synced_board` は、前回読んでから打たれた手が触れた線だけを先に計算し直す。剣先を返す順序は `structures` / `structures_on` とまったく同じなので、探索する木は全走査した場合と変わらない。
