...
| [02-board-implementation.ja.md](02-board-implementation.ja.md) / [en](02-board-implementation.en.md) | `src/board/` が盤面をどう表現し、上のルールをどう実装しているか: 線のビット表現、セグメントとシーケンスの検出、禁手判定、ハッシュ。 |
...
- `src/board/` のルール周り（シーケンス、禁手）を変更するなら 02 を読む。
...
- コードを説明するときは実際の識別子（`Grid::sequences_on`、`SequenceKind::Sword` など）を書き、`grep` で突き合わせられるようにする。
